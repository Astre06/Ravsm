...
def run_playwright(server_num, range_start, range_end, bot, chat_id, loop):
    with sync_playwright() as p:
        ...
        try:
            ...
            # Click login button
            page.wait_for_selector('.button_primary_mod')
            page.click('.button_primary_mod')
            # Click Full Access button
            page.wait_for_selector("//button[contains(., 'Full Access')]")
            page.click("//button[contains(., 'Full Access')]")
        except PlaywrightTimeoutError as e:
            print(f"Error during login sequence, aborted early: {e}")
            browser.close()
            return

        # Now the generation steps run after successful login
        ...
        while seq_num <= max_seq:
            try:
                ...
                while True:
                    try:
                        ...
                        if not trash_buttons:
                            break
                        # Use JavaScript click to mimic Selenium's execute_script click
                        page.evaluate('(element) => element.click()', trash_buttons[0])
                        time.sleep(1)
                        try:
                            ...
                            break
                    except PlaywrightError:  # covers Stale element type errors or similar in Playwright
                        time.sleep(1)
                        continue
                ...
                print(f"Error during generation for seq_num {seq_num}: {e}")
                seq_num += 1
